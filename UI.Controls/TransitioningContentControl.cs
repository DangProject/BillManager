...
namespace UI.Controls
{
    ...
    public class TransitioningContentControl : ContentControl
    {
        ...
        //}
        public ModernUITransitions Transition
        {
            get { return (ModernUITransitions)GetValue(TransitionProperty); }
            set { SetValue(TransitionProperty, value); }
        }
        ...
        public static readonly DependencyProperty TransitionProperty =
            DependencyProperty.Register(
                "Transition",
                typeof(ModernUITransitions),
                typeof(TransitioningContentControl),
                new FrameworkPropertyMetadata(ModernUITransitions.DefaultTransition, FrameworkPropertyMetadataOptions.AffectsRender, OnTransitionPropertyChanged));
                //new PropertyMetadata(ModernUITransitions.DefaultTransition, OnTransitionPropertyChanged));
        ...
        {
            TransitioningContentControl source = (TransitioningContentControl)d;
            ModernUITransitions oldTransition = (ModernUITransitions)e.NewValue;
            ModernUITransitions newTransition = (ModernUITransitions)e.NewValue;
            ...
            // find new transition
            Storyboard newStoryboard = source.GetStoryboard(newTransition.ToString());
            ...
            if (newStoryboard == null)
            {
                ...
                else
                {
                    ...
                    throw new ArgumentException(
                        string.Format(CultureInfo.CurrentCulture, "Transition '{0}' was not defined.", newTransition.ToString()));
                }
            }
            else
            {
        ...
        public override void OnApplyTemplate()
        {
            ...
            Storyboard transition = GetStoryboard(Transition.ToString());
            CurrentTransition = transition;
            if (transition == null)
            {
                string invalidTransition = Transition.ToString();
                // revert to default
                //Transition = DefaultTransitionState;
                Transition = ModernUITransitions.DefaultTransition;
        ...
        private void StartTransition(object oldContent, object newContent)
        {
            ...
            if (CurrentContentPresentationSite != null && PreviousContentPresentationSite != null)
            {
                ...
                if (!IsTransitioning || RestartTransitionOnContentChange)
                {
                    ...
                    VisualStateManager.GoToState(this, NormalState, false);
                    VisualStateManager.GoToState(this, Transition.ToString(), true);
                }
            }
        }

        public string RestartTransitionOnObjectChange
        {
            get { return (string)GetValue(RestartTransitionOnObjectChangeProperty); }
            set { SetValue(RestartTransitionOnObjectChangeProperty, value); }
        }

        public static readonly DependencyProperty RestartTransitionOnObjectChangeProperty =
            DependencyProperty.Register("RestartTransitionOnObjectChange", typeof(object), typeof(TransitioningContentControl), 
            new PropertyMetadata(new object(), OnRestartTransitionOnObjectChangePropertyChanged));

        private static void OnRestartTransitionOnObjectChangePropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ((TransitioningContentControl)d).OnRestartTransitionOnObjectChange(e.OldValue, e.NewValue);
        }

        private void OnRestartTransitionOnObjectChange(object oldValue, object newValue)
        {
            // both presenters must be available, otherwise a transition is useless.
            // old value must be different from new value
            if (CurrentContentPresentationSite != null && PreviousContentPresentationSite != null)
            {
